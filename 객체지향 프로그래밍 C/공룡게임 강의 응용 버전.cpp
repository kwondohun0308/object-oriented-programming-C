...
//나무를 그리는 함수
void DrawTree(int treeX, int randnum)
{
	int axis = TREE_BOTTOM_Y - randnum;
	GotoXY(treeX, axis);
	printf("$$$$");
	GotoXY(treeX, axis + 1);
	printf(" $$ ");
...
int main()
{
	...
	while (true)		//(v2.0) 게임 루프
	{
		...
		while (true)	//한 판에 대한 루프
		{
			...
			//z키가 눌렸고, 바닥이 아닐때 점프
			int keyVal = GetKeyDown();
			if (keyVal == 'z' && isBottom)  
			{ /*그냥  GetKeyDown으로 하면 잘 안먹으므로 나온 값을 다시 변수에 지정하여 그 변수를 기준으로 키를 입력받도록
				먼저 z if문을 들어가서 만약 z키가 아니라면 GetKeyDown에서는 0을 계속 리턴한다. 그러므로 x if문을 들어갔을때
				맞지 않는 이유이다. x키를 꾹 누르고 있을 때 한번씩 되는 이유로는 = 0을 리턴하긴 하지만 무조건 0을 리턴하지 않고 무한루프를 돈다.
				하지만 만약 x의 아스키코드가 리턴이 됐을 때 x if문에 들어가서 점프가 된다.*/
				isJumping = true;
				isBottom = false;
				MaxJumpY = 3;
			}
			
			if (keyVal == 'x' && isBottom)
			{
				isJumping = true;
				isBottom = false;
				MaxJumpY = 0;
			...
			DrawTree(treeX, rannum);		//draw Tree
