...
#define DINO_BOTTOM_Y 19
#define TREE_BOTTOM_Y 28
...
void DrawGameOver(const int score, const int level)
{
	system("cls");
	int x = 18;
	int y = 8;
	...
	GotoXY(x - 10, y + 10);
	printf("If you want to stop playing this game, press 'c' or 'C' key two times");
	printf("\n\n\n\n\n\n\n\n\n");
	system("pause");
}
...
int main()
{
	...
	while (true)		//(v2.0) 게임 루프
	{
		...
		while (true)	//한 판에 대한 루프
		{
			...
			if (tree_1or2 == 0){
				DrawTree1(treeX);		//draw Tree1
			}
			else if (tree_1or2 == 1){
				DrawTree2(treeX);		//draw Tree2
			}
		...
		if (GetKeyDown() == 67 || GetKeyDown() == 99){
			exit(0);
		}
	return 0;
	}
